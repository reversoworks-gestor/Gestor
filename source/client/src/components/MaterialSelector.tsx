...
import { Check, ChevronDown, CircleDollarSign, FileText, Printer, Save, ShieldAlert, X } from "lucide-react";
import type { Material, Order, Printer as PrinterModel } from "@/lib/models";
...
type Props = {
  ...
  onAddToDocument: (payload: Record<string, unknown>, documentId: string) => Promise<void>;
...
export default function MaterialSelector({ stock, printers, documents, onSave, onAddToDocument, onNotice }: Props) {
  const [form, setForm] = useState<SelectorForm>(defaultSelectorForm);
  const [advanced, setAdvanced] = useState(false);
  const [saveDialogOpen, setSaveDialogOpen] = useState(false);
  const [saveDestination, setSaveDestination] = useState<"document" | "order" | null>(null);
  ...
  const payload = useMemo(() => ({ id: `selector-${crypto.randomUUID()}`, project: form.project || "Projeto sem nome", inputs: form, preset: form.preset, recommendation: result.primary?.name || "NO SAFE RECOMMENDATION", score: result.primary?.score || 0, confidence: result.confidence, ranking: result, algorithmVersion: result.algorithmVersion, createdAt: new Date().toISOString() }), [form, result]);
  ...
  const confirmSave = async () => {
    ...
    if (saveDestination === "order" && !selectedDocumentId) return;
    setSaving(true);
    try {
      if (saveDestination === "document") {
        await onSave(payload);
        onNotice("Documento técnico salvo no histórico do Gestor.");
      } else {
        await onAddToDocument(payload, selectedDocumentId);
      }
      setSaveDialogOpen(false);
    } catch {
      onNotice("Não foi possível concluir o salvamento da triagem.");
    } finally {
      setSaving(false);
    }
  };
